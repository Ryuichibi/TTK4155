...
void encoder_init(void)
{
    ...
    REG_PMC_PCER0 |= PMC_PCER0_PID27 | PMC_PCER0_PID28 | PMC_PCER0_PID29 |
                     PMC_PCER0_PID30 | PMC_PCER0_PID31;
    REG_PMC_PCER1 |=
        PMC_PCER1_PID32 | PMC_PCER1_PID33 | PMC_PCER1_PID34 | PMC_PCER1_PID35;
...
void motor_init(void)
{
    ...
    PWM->PWM_CH_NUM[0].PWM_CPRD =
        PERIOD_PWM_MOTOR; // setting period according to define in header
    PWM->PWM_CH_NUM[0].PWM_CDTY =
        PERIOD_PWM_MOTOR; // setting duty cycle to 100% for startup
...
void read_encoder(int32_t *value) { *value = TC2->TC_CHANNEL[0].TC_CV; }

void set_motor_speed(int16_t speed, bool go_left)
{
    PWM->PWM_CH_NUM[0].PWM_CDTYUPD =
        (PERIOD_PWM_MOTOR * (-1 * (speed - 100))) / 100;
    if (go_left) {
        PIOC->PIO_CODR |= PIO_CODR_P23;
    } else {
