...
void print_menu(menu *menu)
{
  ...
  {
    framebuffer_oled_goto_row(i);
    framebuffer_oled_goto_column(0x09);
    framebuffer_oled_print(menu->sub_menus[i]->value);
    
    
    //oled_print(menu->sub_menus[i]->value, strlen(menu->sub_menus[i]->value));
  }
    framebuffer_oled_arrow(menu->selected);
...
{       
        ...
        {
          framebuffer_oled_arrow_reset(current_menu->selected);
          current_menu->selected = (current_menu->selected == 0) ? current_menu->n_entries - 1 : current_menu->selected - 1;
          framebuffer_oled_arrow(current_menu->selected);
        }
